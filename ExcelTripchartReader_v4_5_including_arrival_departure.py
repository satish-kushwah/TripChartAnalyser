...
def myfunction(mychoice,totalduties):
	...
	BCGNDnCol=11; BCGNUpCol=15 ;JPWUpCol=21; JPWDepCol=5

	import openpyxl,time,pprint
	#mychoice=input('Enter 1 for WEEK, 2 for SAT or 3 for SUN: ')
	if mychoice==1:
		f=open(App_data_path+'WeekTripsData.py','w') # database name according to tt
		...
		f=open(App_data_path+'SatTripsData.py','w') # database name according to tt
		dictname='sattrips' #dictionary name
		tt=openpyxl.load_workbook(tripchart_path+'SAT.xlsx') # excel file name
		#ttsheet=tt['TC'] # sheet name
	elif mychoice==3:	
		f=open(App_data_path+'SunTripsData.py','w') # database name according to tt
	...
	for k in range(1,totalduties+1): 
		...
		for i in range(totalrows): #reading rows of excels
			...
			#Arrival to BCGN
			if ttsheet.cell(row=i+firstrow,column=BCGNDnCol+1).value==duty:
				trainTime=ttsheet.cell(row=i+firstrow,column=BCGNDnCol).value
				if trainTime==None:
					trainTime='Unknown'
				trip.append('Arrival'); trip.append(str(trainTime)[-8:-3]+'/BCGN_DN'); trip.append(str(ttsheet.cell(row=i+firstrow,column=trainNoCol).value))
				alltrips.append(trip)
				trip=[]
			#Departure from BCGN
			if ttsheet.cell(row=i+firstrow,column=BCGNUpCol+1).value==duty:
				trainTime=ttsheet.cell(row=i+firstrow,column=BCGNUpCol).value
				if trainTime==None:
					trainTime='Unknown'
				trip.append('Departure'); trip.append(str(trainTime)[-8:-3]+'/BCGN_UP'); trip.append(str(ttsheet.cell(row=i+firstrow,column=trainNoCol).value))
				alltrips.append(trip)
				trip=[]
			#Arrival at JPW
			if ttsheet.cell(row=i+firstrow,column=JPWUpCol).value!=None and ttsheet.cell(row=i+firstrow,column=JPWUpCol+1).value==duty:
				trainTime=ttsheet.cell(row=i+firstrow,column=JPWUpCol).value
				if trainTime==None:
					trainTime='Unknown'
				trip.append('Arrival'); trip.append(str(trainTime)[-8:-3]+'/JPW_UP'); trip.append(str(ttsheet.cell(row=i+firstrow,column=trainNoCol).value))
				alltrips.append(trip)
				trip=[]
			#Departure from JPW
			if ttsheet.cell(row=i+firstrow,column=JPWDepCol+1).value!=None and ttsheet.cell(row=i+firstrow,column=JPWDepCol+1).value==duty:
				trainTime=ttsheet.cell(row=i+firstrow,column=JPWDepCol).value
				if trainTime==None:
					trainTime='Unknown'
				trip.append('Departure'); trip.append(str(trainTime)[-8:-3]+'/JPW_DN'); trip.append(str(ttsheet.cell(row=i+firstrow,column=trainNoCol).value))
				alltrips.append(trip)
				trip=[]
